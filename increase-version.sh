...
SRC_PKGNAME="dns-admin"
PKG_LIBNAME="dns_admin"
...
case "${CUR_GIT_BRANCH}" in
    ...
    hotfix/*) WANT_STAGE="stable";;
    hf/*) WANT_STAGE="stable";;
...
my_debchange() {
    ...
    echo "${SRC_PKGNAME} (${version}) ${stage}; urgency=low" >"${TMP_FILE}"
    echo >>"${TMP_FILE}"
    for entry in "$@"; do
        if [ "${entry}" != "" ] ; then
            echo "  * ${entry}" >>"${TMP_FILE}"
...
scripts="src/${PKG_LIBNAME}/__init__.py"
